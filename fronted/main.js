...
window.addEventListener('DOMContentLoaded',(event)=>{
    ...
    axios.get("http://localhost:4000/fetchdata")
    .then((Response)=>{
      console.log(Response);
      for(var i=0;i<Response.data.data.length;i++)
      {
        displaydata(Response.data.data[i]);
        console.log("hello");
      }
    }).catch((error)=>{
      console.log(error)
...
function  displaydata(obj){
    ...
    const li=`<li id=${obj.id}>${obj.nam}-${obj.email}
    <button onclick=deletedata('${obj.id}')>delete</button>
    <button onclick=editdata('${obj.id}','${obj.nam}','${obj.email}')>edit</button></li>`;
...
function deletedata(deleteid)
{
   // localStorage.removeItem(deleteid);
    ...
    axios.delete(`http://localhost:4000/deleteuser/${deleteid}`)
    .then((response)=>{
      removedata(deleteid);
      console.log(response);
    }).catch((err)=>{
